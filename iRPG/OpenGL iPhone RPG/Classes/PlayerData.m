...
@synthesize hp, hpmax, mp, mpmax, lvl, exp, gold, sprite, displayToggle, currentMap, playerTilePos, movedToDir, att, def, weapon, armor, trinket, potions, playerName,playerClass;
- (id)init 
{
	//if (self = [super init])
	att = 1;
...
{   
	...
	[coder encodeObject: [NSNumber numberWithFloat:movedToDir.x] forKey:@"movedToDir.x" ];
	[coder encodeObject: [NSNumber numberWithFloat:movedToDir.y] forKey:@"movedToDir.y" ];
...
- (id) initWithCoder: (NSCoder *) coder
{
	...
	movedToDir.x = [[coder decodeObjectForKey:@"movedToDir.x"] floatValue];
	movedToDir.y = [[coder decodeObjectForKey:@"movedToDir.y"] floatValue];
