/**
 ~Maybe see if you can make the drawing thread asynchronous so have a better loading screen.
...
- (id)initWithCoder:(NSCoder*)coder {
    ...
    if ((self = [super initWithCoder:coder])) {
		...
		gameTimer = [NSTimer scheduledTimerWithTimeInterval:(float)1/40 target:self selector:@selector(mainDrawLoop) userInfo:nil repeats:YES];
		[self startLogic];
...
- (void)mainGameLoop {
	...
	if (currentView == GameScreen) {
		...
		if (movePlayerTimer != movePlayerTimerMax) {
			movePlayerTimer += 1;
			updateScreen = TRUE;
		} else if (playerData.movedToDir.x != 0 || playerData.movedToDir.y != 0) {playerData.movedToDir = CGPointMake(0, 0);}
		//Player Attack Timer
		...
		for (int i = 0; i < [npcs count]; i ++) {
			...
			if (npcInMap) {
				...
				//Player Move Timer
				if (npc.moveNpcTimer != moveNpcTimerMax) {
					npc.moveNpcTimer += 1;
					updateScreen = TRUE;
				} else if (npc.movedToDir.x != 0 || npc.movedToDir.y != 0) {npc.movedToDir = CGPointMake(0, 0);}
	...
	if (currentView == Loading) {
		...
		[characterImages addObject:[tiles objectAtIndex:0]];
		for (int e = 1;e > -1;e += 1) {
			loadingProgress += 1;
			[self renderScene];
		...
		[itemImages addObject:[tiles objectAtIndex:0]];
		for (int e = 1;e > -1;e += 1) {
			loadingProgress += 1;
			[self renderScene];
	...
	switch (currentView) {
		...
		case GameScreen:
		{
			...
			if (playerData.currentMap > -1 && playerData.currentMap < [maps count]) {
				CGPoint playerMoveOffset = 
				CGPointMake(round(playerData.movedToDir.x*((float)(movePlayerTimerMax-movePlayerTimer)*45/movePlayerTimerMax)),
							round(playerData.movedToDir.y*((float)(movePlayerTimerMax-movePlayerTimer)*45/movePlayerTimerMax)));
				//NSLog(@"%f %f", playerMoveOffset.x, playerMoveOffset.y);
				MapData* map[9];
				int mapNumbers[9];
				map[Center] = [maps objectAtIndex:playerData.currentMap]; //Selected Map
				[self getSurroundingMaps:&map];
				...
				for (int l = 0; l < 10; l ++) {
					...
					for (int i = 0; i < 9; i ++) {
						if (map[i] != nil) {
							...
							int mapy = addY-playerData.playerTilePos.y*45+middleScreen.y/2;
							//Only draw maps in view
							if (mapx < screenDimensions.x-45 && mapx + mapWidth + 45 > 0 &&
								...
								if (l == 6) { //Before fringe draw player
									...
									for (int e = 0; e < [npcs count]; e ++) {
										...
										if (mapNumbers[i] == npc.currentMap) {
											...
											CGPoint npcMoveOffset = CGPointMake(round(npc.movedToDir.x*((float)(moveNpcTimerMax-npc.moveNpcTimer)*45/moveNpcTimerMax)),
														round(npc.movedToDir.y*((float)(moveNpcTimerMax-npc.moveNpcTimer)*45/moveNpcTimerMax)));
											CGPoint npcPosition = CGPointMake(round(npc.currentTile.x*45 - playerData.playerTilePos.x*45+addX+middleScreen.x/2)+playerMoveOffset.x - npcMoveOffset.x,round(npc.currentTile.y*45 - playerData.playerTilePos.y*45+addY+middleScreen.y/2)+playerMoveOffset.y-npcMoveOffset.y);
									...
									for (int d = 0; d < [damageEffects count]; d += 1) {
										...
										if (mapNumbers[i] == damage.mapPosition) {
											...
											[self drawImage:damage.textImage AtPoint: CGPointMake(round(damage.tilePosition.x*45 - playerData.playerTilePos.x*45+addX+middleScreen.x/2+.5*45)+playerMoveOffset.x,round(damage.tilePosition.y*45 - playerData.playerTilePos.y*45+addY+middleScreen.y/2+.5*45 - (damage.ticks/damage.ticksMax*0.6)*45)+playerMoveOffset.y)];
								...
								//Draw the tiles.
								int startx = floor((playerData.playerTilePos.x*45 - screenDimensions.x/2-addX-playerMoveOffset.x)/45);
								if (startx < 0) {startx = 0;}
								int endx = ceil((playerData.playerTilePos.x*45 + screenDimensions.x/2-addX-playerMoveOffset.x)/45);
								if (endx > [maptiles count]) {endx = [maptiles count];}
								for (int x = startx; x < endx; x ++) {
									NSArray* column = [maptiles objectAtIndex:x];
									int starty = floor((playerData.playerTilePos.y*45 - screenDimensions.y/2-addY-playerMoveOffset.y)/45);
									if (starty < 0) {starty = 0;}
									int endy = ceil((playerData.playerTilePos.y*45 + screenDimensions.y/2-addY-playerMoveOffset.y)/45+1);
									if (endy > [column count]) {endy = [column count];}
									for (int y = starty; y < endy; y ++) {
										TileData* tile = [column objectAtIndex:y];
										if ([tile getTileOnLayer:l] > 0) {
											[self drawImage:[tiles objectAtIndex:[tile getTileOnLayer:l]] AtPoint:CGPointMake(round(x*45+addX-playerData.playerTilePos.x*45+middleScreen.x/2)+playerMoveOffset.x,round(y*45+addY-playerData.playerTilePos.y*45+middleScreen.y/2)+playerMoveOffset.y)];
										}
									}
								}
							}
...
- (void)mainDrawLoop {
	...
	//NSLog(@"Main Draw Loop is running");
	if (updateScreen) {updateScreen = FALSE; [self renderScene];}
	//[drawThread sleepForTimeInterval:4];
}
...
{ 
	while (runningLogic) {
		NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
		while(CFRunLoopRunInMode(kCFRunLoopDefaultMode, (1.0/20.0), FALSE) == kCFRunLoopRunHandledSource);
		//[self mainGameLoop];
		[self mainGameLoop];
		[pool release];
	}
...
{
	runningLogic = TRUE;
	[self performSelectorOnMainThread:@selector(runLogic) withObject:nil waitUntilDone:NO];
}
...
- (void)movePlayer:(int)direction {
	playerData.movedToDir = CGPointMake(0, 0);
	...
	if (newMapPos > -1 && newMapPos < [maps count]) {
		...
		if (!newTile.blocked) {
			...
			for (int i = 0; i < [npcs count]; i ++) {
				WorldNpc* npc = [npcs objectAtIndex:i];
				NpcData* dataOfNpc = [npcData objectAtIndex:npc.npcNum];
				if (npc.currentTile.x == newPosition.x && npc.currentTile.y == newPosition.y && npc.currentMap == newMapPos && dataOfNpc.attackStyles != AttackNothing) {blockedNpc = i;}
			}
			if (blockedNpc == -1) { //No npc so move
				if (direction == 1) {playerData.movedToDir = CGPointMake(-1, 0);}
				if (direction == 2) {playerData.movedToDir = CGPointMake(1, 0);}
				if (direction == 3) {playerData.movedToDir = CGPointMake(0, -1);}
				if (direction == 4) {playerData.movedToDir = CGPointMake(0, 1);}
...
- (void)moveNpc:(int)npcNum direction:(int)direction {
	...
	if (newMapPos > -1 && newMapPos < [maps count]) {
		...
		if (!newTile.blocked) {
			//Allow no npc to walk on top of each other.
			BOOL npcInWay = FALSE;
			for (int n = 0; n < [npcs count]; n++) {
				WorldNpc* npcCollide = [npcs objectAtIndex:n];
				NSLog(@"%d %d vs %d %f vs %f %f vs %f",n, npcCollide.currentMap, newMapPos, npcCollide.currentTile.x, newPosition.x, npcCollide.currentTile.y, newPosition.y);
				if (npcCollide.currentMap == newMapPos && npcCollide.currentTile.x == newPosition.x && npcCollide.currentTile.y == newPosition.y) {
					NSLog(@"Blocked NPC");
					npcInWay = TRUE;
				}
			}
			if (npcInWay == FALSE && playerData.currentMap != newMapPos || playerData.playerTilePos.x != newPosition.x || playerData.playerTilePos.y != newPosition.y) {
				if (direction == 1) {npc.movedToDir = CGPointMake(-1, 0);}
				if (direction == 2) {npc.movedToDir = CGPointMake(1, 0);}
				if (direction == 3) {npc.movedToDir = CGPointMake(0, -1);}
				if (direction == 4) {npc.movedToDir = CGPointMake(0, 1);}
				npc.moveNpcTimer = 0;
				npc.currentTile = newPosition;
				npc.currentMap = newMapPos;
				updateScreen = TRUE;
			} else if (npcsData.attackStyles == AttackPlayer) {
